...
import type { ErrorResponse, SuccessMessageResponse } from '../support/types/api';
  ...
  context('Admin', () => {
    ...
    it('Deve cadastrar um novo carrinho', () => {
      ...
      Cypress._.times(3, () => {
        ...
        createProductId().then((id: string) => {
          productIds.push(id);
        });
      });
      ...
      cy.wrap(productIds)
        ...
        .then(() => {
          apiRequest({
            ...
            expect((response.body as SuccessMessageResponse).message).to.equal('Cadastro realizado com sucesso');
          });
        });
    });
    ...
    it('Deve ser exibido uma mensagem caso tente colocar um produto inexistente no carrinho', () => {
      apiRequest({
        ...
        expect((response.body as ErrorResponse).message).to.equal('Produto não encontrado');
      });
    });

    it('Deve ser exibido uma mensagem caso tente colocar dois produtos iguais no carrinho', () => {
      createProductId().then((productId: string) => {
        apiRequest({
          method: 'POST',
          url: '/carrinhos',
          failOnStatusCode: false,
          ...
          expect(response.status).to.equal(400);
          expect((response.body as ErrorResponse).message).to.equal('Não é permitido possuir produto duplicado');
        });
      });
    });

    it('Deve ser exibido uma mensagem caso tente colocar um produto com quantidade zerada no carrinho', () => {
      createProductId(1).then((productId: string) => {
        apiRequest({
          method: 'POST',
          url: '/carrinhos',
          failOnStatusCode: false,
          ...
          expect(response.status).to.equal(400);
          expect((response.body as ErrorResponse).message).to.equal('Produto não possui quantidade suficiente');
        });
      });
    });
    ...
    it('Deve ser exibido uma mensagem caso tente cadastrar sem autenticação de usuário administrador', () => {
      apiRequest({
        ...
        expect((response.body as ErrorResponse).message).to.equal('Token de acesso ausente, inválido, expirado ou usuário do token não existe mais');
      });
    });

    it('Deve buscar um carrinho de compras', () => {
      createTrolley().then((trolleyId: string) => {
        apiRequest({
          method: 'GET',
    ...
    it('Deve ser exibido uma mensagem caso o carrinho não seja encontrado', () => {
      apiRequest({
        ...
        expect(response.status).to.equal(400);
        expect((response.body as ErrorResponse).message).to.equal('Carrinho não encontrado');
      });
    });
    ...
    it('Deve concluir com compra e excluir o carrinho', () => {
      createTrolley().then(() => {
        apiRequest({
          ...
          expect(response.status).to.equal(200);
          expect((response.body as SuccessMessageResponse).message).to.equal('Registro excluído com sucesso');
        });
      });
    });
    ...
    it('Deve ser exibido uma mensagem caso não encontre o carrinho do usuário', () => {
      apiRequest({
        ...
        expect(response.status).to.equal(200);
        expect((response.body as SuccessMessageResponse).message).to.equal('Não foi encontrado carrinho para esse usuário');
      });
    });
    ...
    it('Deve cancelar uma compra', () => {
      createTrolley().then(() => {
        apiRequest({
          ...
          expect(response.status).to.equal(200);
          expect((response.body as SuccessMessageResponse).message).to.equal('Registro excluído com sucesso. Estoque dos produtos reabastecido');
        });
      });
    });
