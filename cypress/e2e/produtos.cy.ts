...
import type { ErrorResponse, ProductResponse, SuccessMessageResponse } from '../support/types/api';
...
describe('Testes de produtos via API', () => {
  context('Admin', () => {
    ...
    it('Deve listar todos os produtos cadastrados', () => {
      apiRequest({ method: 'GET', url: '/produtos' }).then((response: Cypress.Response<unknown>) => {
    ...
    it('Deve exibir mensagem ao usar token inválido ou expirado', () => {
      apiRequest({
        ...
        expect((response.body as ErrorResponse).message).to.include('Token de acesso ausente, inválido, expirado');
      });
    });
    ...
    it('Deve buscar um produto pelo ID', () => {
      createProductId().then((productId: string) => {
        apiRequest({
          ...
          expect(response.status).to.equal(200);
          expect((response.body as ProductResponse)._id).to.equal(productId);
        });
      });
    });
    ...
    it('Deve exibir mensagem ao não encontrar produto', () => {
      apiRequest({
        ...
        expect(response.status).to.equal(400);
        expect((response.body as ErrorResponse).message).to.include('Produto não encontrado');
      });
    });
  ...
  context('Normal', () => {
    ...
    it('Deve exibir mensagem de rota exclusiva ao tentar cadastrar produto com usuário normal', () => {
      apiRequest({
        ...
        expect((response.body as ErrorResponse).message).to.include('Rota exclusiva para administradores');
      });
    });
